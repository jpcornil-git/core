...
from .const import DEVICE_CLASS_UV_INTENSITY, DOMAIN
...
_SENSOR_SCHEMA = vol.Schema(
    {
        vol.In([DEVICE_CLASS_UV, DEVICE_CLASS_UV_INTENSITY]): cv.string,
    }
)
    ...
    if i2c_address not in hass.data[DOMAIN]:
        try:
            ...
            hass.data[DOMAIN][i2c_address] = await hass.async_add_executor_job(
                functools.partial(
                    VEML6075, bus, i2c_address, config[CONF_SCAN_SLOWDOWN]
                )
            )
        ...
        for sensor_class, sensor_name in sensor_devices.items():
            sensor_entity = VEML6075Sensor(hass, config, sensor_class, sensor_name)
