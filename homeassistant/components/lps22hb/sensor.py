...
from homeassistant.components.lps22hb import LPS22HB
...
_SENSOR_SCHEMA = vol.Schema(
    {
        vol.In([DEVICE_CLASS_PRESSURE, DEVICE_CLASS_TEMPERATURE]): cv.string,
    }
)
    ...
    if i2c_address not in hass.data[DOMAIN]:
        try:
            ...
            hass.data[DOMAIN][i2c_address] = await hass.async_add_executor_job(
                functools.partial(LPS22HB, bus, i2c_address, config[CONF_SCAN_SLOWDOWN])
            )
        ...
        for sensor_class, sensor_name in sensor_devices.items():
            sensor_entity = LPS22HBSensor(hass, config, sensor_class, sensor_name)
...
class LPS22HBSensor(Entity):
    ...
    def unit_of_measurement(self):
        ...
        if self._device_class == DEVICE_CLASS_PRESSURE:
