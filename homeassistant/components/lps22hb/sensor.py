...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for sensor_class, sensor_name in sensor_devices.items():
        sensor_entity = LPS22HBSensor(sensor_class, sensor_name)
        if await hass.async_add_executor_job(
            functools.partial(
                sensor_entity.bind, LPS22HB, i2c_bus, i2c_address, scan_slowdown
            )
        ):
...
class LPS22HBSensor(Entity):
    ...
    def bind(self, device_class, bus, address, scan_slowdown):
        ...
        if self._device:
            if self._device_class == DEVICE_CLASS_PRESSURE:
                sensor_function = self._device.get_pressure
            elif self._device_class == DEVICE_CLASS_TEMPERATURE:
